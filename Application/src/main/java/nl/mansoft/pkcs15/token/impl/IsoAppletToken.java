package nl.mansoft.pkcs15.token.impl;
...
//        }
    ...
    public DF createDF(int path, long size, DFAcl acl) throws IOException {
        ...
        CommandApdu cmd = new CommandApdu((byte) 0x00, (byte) 0xE0, (byte) 0x00, (byte) 0x00, data, DEFAULT_LE);

        try {
            ResponseApdu resp = mSmartcardIO.runAPDU(cmd);

            if (resp.getSwValue() != PKCS15Exception.ERROR_OK)
                throw new PKCS15Exception("CREATE FILE for DF ["+PathHelper.formatPathAppend(this.currentFile.getPath(),path)+"] returned error",resp.getSwValue());
...
//        dos.write(acl.getAcDecrease());
        ...
        try {
            ...
            if (resp.getSwValue() != PKCS15Exception.ERROR_OK)
                throw new PKCS15Exception("CREATE FILE for EF ["+PathHelper.formatPathAppend(this.currentFile.getPath(),path)+"] returned error",resp.getSwValue());
    ...
    public void deleteDF(int path) throws IOException {
        ...
        try {
            ...
            if (resp.getSwValue() != PKCS15Exception.ERROR_OK)
                throw new PKCS15Exception("DELETE FILE for DF ["+PathHelper.formatPathAppend(this.currentFile.getPath(),path)+"] returned error",resp.getSwValue());
    ...
    public void deleteEF(int path) throws IOException {
        ...
        try {
            ...
            if (resp.getSwValue() != PKCS15Exception.ERROR_OK)
                throw new PKCS15Exception("DELETE FILE for EF ["+PathHelper.formatPathAppend(this.currentFile.getPath(),path)+"] returned error",resp.getSwValue());
    ...
    {
        if (resp.getSwValue() != PKCS15Exception.ERROR_OK)
            throw new PKCS15Exception("Card error in response to SELECT FILE",resp.getSwValue());
    ...
    private class EFOutputStream extends ByteArrayOutputStream {
        ...
        public void flush() throws IOException {
            ...
            try {
                ...
                if (resp.getSwValue() != PKCS15Exception.ERROR_OK)
                    throw new PKCS15Exception("UPDATE BINARY for EF ["+this.pathToWrite+"] returned error",resp.getSwValue());
